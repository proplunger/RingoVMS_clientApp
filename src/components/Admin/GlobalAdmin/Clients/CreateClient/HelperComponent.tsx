...
import { faPlusCircle, faTrash, faPencilAlt, faSave, faUndo, faTimesCircle, faCheckCircle } from "@fortawesome/free-solid-svg-icons";



export function CustomHeaderActionCell({ add, ExportMenu }) {
...
export function CustomHeaderActionCellRegion({ add, ExportMenu }) {
    return class extends GridHeaderCell {
        handleHeaderMenuClick = (actionValue: number) => {
            console.log("Action Taken " + actionValue);
        };
        ...
        render() {
            ...
            const contentAddRender = (props) => {
                return (
                    <div className="pb-1 pt-1 w-100 myorderGlobalicons" onClick={() => add(props.dataItem)}>
                        <FontAwesomeIcon icon={faPlusCircle} className={"nonactive-icon-color ml-2 mr-2"}></FontAwesomeIcon> Add Region{" "}
                    </div>
                );
            }
...
export function CustomHeaderActionCellZone({ add, ExportMenu }) {
  return class extends GridHeaderCell {
      handleHeaderMenuClick = (actionValue: number) => {
          console.log("Action Taken " + actionValue);
      };
      ...
      render() {
          ...
          const contentAddRender = (props) => {
              return (
                  <div className="pb-1 pt-1 w-100 myorderGlobalicons" onClick={() => add(props.dataItem)}>
                      <FontAwesomeIcon icon={faPlusCircle} className={"nonactive-icon-color ml-2 mr-2"}></FontAwesomeIcon> Add Zone{" "}
                  </div>
              );
          }
...
export function CustomHeaderActionCellAssociatedVendor({ add, ExportMenu }) {
    return class extends GridHeaderCell {
        handleHeaderMenuClick = (actionValue: number) => {
            console.log("Action Taken " + actionValue);
        };
        ...
        render() {
            ...
            const contentAddRender = (props) => {
                return (
                    <div className="pb-1 pt-1 w-100 myorderGlobalicons" onClick={() => add(props.dataItem)}>
                        <FontAwesomeIcon icon={faPlusCircle} className={"nonactive-icon-color ml-2 mr-2"}></FontAwesomeIcon> Associate Vendor{" "}
                    </div>
                );
            }
...
export function MyCommandCell({ edit, remove,  update,  cancel, editField, add, discard, inactive, active }) {
  ...
  return inEdit ? (
      <td className="k-command-cell  k-command-cell-icon" contextMenu="Action">
       <div className="txt-clr-blue">
        <span
        className={"font-medium"+ ((dataItem.name && dataItem.name.trim() && dataItem.name.length <= 100) || dataItem.vendorId ? " " : " disable-opacity")}
      ...
      {isNewItem && dataItem.status=="Draft" ? null  :
        ...
        onClick={() => (dataItem.status=="Active" ? inactive(dataItem,0) : active(dataItem,1))}
      >
