...
import { ICreateRequisitionState, ICreateRequisitionProps } from "./models/ICreateRequisitionState";
...
import { requisitionValidation, validate } from "./validations/validation";
...
import { EntityType, ReqStatus, SettingCategory } from "../../Shared/AppConstants";
import BreadCrumbs from "../../Shared/BreadCrumbs/BreadCrumbs";

const billRate = 100;

class CreateRequisition extends React.Component<ICreateRequisitionProps, ICreateRequisitionState> {
    public reqChild: any;
    ...
    componentDidMount() {
        ...
        if(auth.hasPermissionV2(AppPermissions.REQ_CREATE)){
    ...
    handleReqApproversChange = (data) => {
        this.setState({ reqApproverDetails: data });
    };

    handleTimesheetApproversChange = (data) => {
    ...
    saveRequisition(isSubmit: boolean) {
        if (this.checkValidations(isSubmit)) {
            ...
            axios[httpVerb]("/api/requisitions", JSON.stringify(data)).then((res) => {
                        ...
                        <div className="row pt-2 pb-2 accordianTitleClr mx-auto mb-3 mt-3">
                            <div className="col-2 col-md-4 fonFifteen paddingLeftandRight">
                                <div className="d-none d-md-block">
                                    <BreadCrumbs globalData={{clientId: this.state.clientId,requisitionId:this.state.reqId}}></BreadCrumbs>
                </div>
                            </div>
                            ...
                            <Form noValidate className="col-12 ml-0 mr-0" id="collapsiblePadding" translate="yes" onChange={formikProps.handleChange}>
                                ...
                                >
                                    ...
                                    <RequisitionDetails
                                        ...
                                        disableDivAndLoc={this.state.status==ReqStatus.APPROVED}
                                ...
                                <div>
                                    ...
                                    >
                                        {this.state.status &&
                                            <ApprovalsWFGrid
                                                ref={(instance) => {
                                                    this.reqApprovalChild = instance;
                                                }}
                                                entityId={reqId}
                                                ...
                                                canEdit={true}
                                                canEditDefault={allowEdit}
                                                disableAddLevel={this.state.status==ReqStatus.APPROVED}
                                                disableActiveLevel={this.state.status !=ReqStatus.DRAFT}
                                                key="Requisition"
                                ...
                                <div className="row mb-2 mb-lg-4  ml-sm-0 mr-sm-0">
                                    <div className="col-12 mt-5 text-sm-center text-center font-regular">
                                        ...
                                        {this.state.status==ReqStatus.DRAFT && auth.hasPermissionV2(AppPermissions.REQ_CREATE) && (
                                            <button
                                                type="button"
                                                disabled={!clientId}
                                                ...
                                                onClick={() => {
                                                    let isValid = true;
                                                    if (!this.state.clientDivisionId) {
                                                        formikProps.setFieldError('clientDivisionId', 'This field is required');
                                                        isValid = false;
                                                    }
                                                    if (!this.state.divisionLocationId) {
                                                        formikProps.setFieldError('divisionLocationId', 'This field is required');
                                                        isValid = false;
                                                    }
                                                    if (this.state.reqPosition.noOfReqStaff > 999) {
                                                        formikProps.setFieldError('reqPosition.noOfReqStaff', 'No of required staff cannot be greater than 999');
                                                        isValid = false;
                                                    }
                                                    if (this.state.reqPosition.noOfReqStaff < 1) {
                                                        formikProps.setFieldError('reqPosition.noOfReqStaff', 'No of required staff cannot be less than 1');
                                                        isValid = false;
                                                    }
                                                    if (this.state.reqPosition.endDate==null) {
                                                        formikProps.setFieldError('reqPosition.endDate', 'This field is required');
                                                        isValid = false;
                                                    }
                                                    if (isValid) {
                                        ...
                                        {this.state.status==ReqStatus.DRAFT && auth.hasPermissionV2(AppPermissions.REQ_CREATE) && (
                                            ...
                                            >
                                                <FontAwesomeIcon icon={faCheckCircle} className={"mr-1"} /> Submit
                                            </button>
                                        )}
                                        {this.state.status !=ReqStatus.DRAFT && auth.hasPermissionV2(AppPermissions.REQ_UPDATE) && (
                                            <button
                                                type="submit"
                                                disabled={!clientId}
                                                className="btn button button-bg shadow mb-2 mb-xl-0"
